...
#ifdef CONFIG_COMPAT
static ksu_syscall_hook_fn compat_syscall_hooks[KSU_NR_COMPAT_SYSCALLS];
#endif
...
static int patch_compat_syscall_table(int nr, syscall_fn_t fn)
{
    if (ksu_compat_syscall_table == NULL)
        return -ENOENT;
    if (nr < 0 || nr >= KSU_NR_COMPAT_SYSCALLS)
        return -EINVAL;
...
static int __init ksu_compat_find_ni_syscall_slots(int *out_slots, int max_slots)
{
    ...
    pr_info("compat sys_ni_syscall: 0x%lx\n", ni_syscall);

    if (!ni_syscall)
        return 0;

    for (i = 0; i < KSU_NR_COMPAT_SYSCALLS && count < max_slots; i++) {
        if ((unsigned long)ksu_compat_syscall_table[i] == ni_syscall) {
            out_slots[count++] = i;
            pr_info("compat ni_syscall %d: %d\n", count, i);
        }
    }
...
#ifdef CONFIG_COMPAT
// Compat dispatcher. r8 is a callee-saved AArch32 register and cannot be used as
// a stash the way the native path uses x8, but r7 already holds the original
// number and the kernel never writes it back, so read it from there. A direct
// call to our slot arrives with r7 == the slot, which is how it is rejected.
static long __nocfi ksu_compat_syscall_dispatcher(const struct pt_regs *regs)
{
    if (regs->syscallno != ksu_compat_dispatcher_nr)
        return -ENOSYS;

    int orig_nr = (int)PT_REGS_COMPAT_SYSCALL(regs);

    if (regs->syscallno == orig_nr)
        return -ENOSYS;

    // syscallno is kernel state, not a user register: signal restart and the
    // sys_exit tracepoint both read it back.
    ((struct pt_regs *)regs)->syscallno = orig_nr;

    if (likely(orig_nr >= 0 && orig_nr < KSU_NR_COMPAT_SYSCALLS)) {
...
#ifdef CONFIG_COMPAT
// Register a handler into the compat dispatcher's routing table.
...
#ifdef CONFIG_COMPAT
// Remove a handler from the compat dispatcher's routing table.
// The syscall table is not touched — only the dispatcher stops routing this nr.
...
#ifdef CONFIG_COMPAT
static void __init ksu_compat_syscall_hook_init(void)
{
    int ni_slot;
    ...
    ksu_compat_syscall_table_hook(ksu_compat_dispatcher_nr, (syscall_fn_t)ksu_compat_syscall_dispatcher, NULL);
    pr_info("compat dispatcher installed at slot %d\n", ksu_compat_dispatcher_nr);
}

static void __exit ksu_compat_syscall_hook_exit(void)
{
    int i;
    ...
    for (i = 0; i < compat_hooked_count; i++) {
        ...
        pr_info("restore compat syscall %d to 0x%lx\n", nr, (unsigned long)orig);
...
#else
static inline void ksu_compat_syscall_hook_init(void)
{
}
static inline void ksu_compat_syscall_hook_exit(void)
{
}
#endif
